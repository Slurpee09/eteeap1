          ...
          <div className="flex flex-col w-full items-center gap-3">
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!ok) return;
                try {
                  await axios.delete('http://localhost:5000/profile/delete', { headers: { 'x-user-id': user?.id }, withCredentials: true });
                } catch (err) {
                  console.error('Delete account failed:', err.response?.data || err.message);
                  alert('Failed to delete account.');
                  return;
                }
                // on success, clear local data and navigate to login
                localStorage.removeItem('user');
                onClose();
                navigate('/login');
              }}
              className="w-full px-6 py-2 bg-red-800 text-white rounded hover:bg-red-900"
            >
              Delete Account
            </button>
          </div>
