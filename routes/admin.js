  ...
  try {
    const [rows] = await db.query(
      `SELECT a.id, DATE_FORMAT(a.created_at, '%Y-%m-%d %h:%i %p') as date,
              u.fullname as user, a.action, a.details
...
router.post("/log", async (req, res) => {
  try {
    const { user_id, role, action, details } = req.body || {};
    const userId = req.user?.id || req.headers["x-user-id"] || user_id || null;
    await logActivity(userId, role || "client", action || "log", details || "");
